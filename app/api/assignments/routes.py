...
async def get_assignments(user_id:str=Depends(get_current_user_id),db=Depends(get_database)):
...
async def get_assignments_all_mission(user_id:str=Depends(get_current_user_id),db=Depends(get_database)):
...
@router.put("/missions/votes/{voter_id}",response_model = EventResponse)
async def Update_assignments_missions_params_vote(voter_id:str,update_data: ParamsUpdateVote,user_id:str=Depends(get_current_user_id),db=Depends(get_database)):

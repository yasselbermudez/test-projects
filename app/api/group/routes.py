...
from .schemas import Group,EventResponse,UpdateGroup,UpdateMembers,CreateGroup
from .service import update_members,update_group,create_group,delete_group_in_cascade,delete_group_by_id

router = APIRouter()

@router.post("/", response_model=Group)
async def create_new_group(group_data: CreateGroup, db=Depends(get_database)):
    result = await create_group(group_data, db)
    return result

@router.put("/{group_id}", response_model=Group)
async def update_group_by_id(group_id: str, update_data: UpdateGroup, db=Depends(get_database)):
    result = await update_group(group_id,update_data,db)
    return result

@router.put("/members/{group_id}", response_model=Group)
async def update_members_group(group_id: str, update_data: UpdateMembers, db=Depends(get_database)):
...
async def delete_group_cascade(group_id: str,db=Depends(get_database)):

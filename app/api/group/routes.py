...
async def create_new_group(group_data: CreateGroup,user=Depends(get_current_user), db=Depends(get_database)):
    result = await create_group(group_data, db)
    return result

@router.put("/{group_id}", response_model=Group)
async def update_group_by_id(group_id: str, update_data: UpdateGroup,user=Depends(get_current_user), db=Depends(get_database)):
    result = await update_group(group_id,update_data,db)
    return result

@router.put("/members/{group_id}", response_model=Group)
async def update_members_group(group_id: str, update_data: UpdateMembers,user=Depends(get_current_user), db=Depends(get_database)):
...
async def get_group_by_id(group_id: str, db=Depends(get_database)):
...
async def delete_group(group_id: str,user=Depends(get_current_user),db=Depends(get_database)):
    return await delete_group_by_id(group_id,db)

@router.delete("/{group_id}/cascade")
async def delete_group_cascade(group_id: str,user=Depends(get_current_user),db=Depends(get_database)):
